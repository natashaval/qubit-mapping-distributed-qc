...
\end{figure}
% Table \ref{tab:table-average-overhead} presents the average gate and depth performance comparison between Lookahead - Basic and Lookahead - Sabre strategies based on grouping. The values are shown as percentages, with higher percentages indicating a greater performance gap between the two strategies. A positive value signifies that \lstinline{LookaheadSwap} introduces fewer additional swap gates than the other strategy, while a negative value indicates the opposite. The term "overhead" refers to the number of logical gates or the depth before and after transpilation, calculated using the formula $G_{\textrm{overhead}} = \frac{(G_{\textrm{after}} - G_{\textrm{before}})}{G_{\textrm{before}}}$. Group 1 and Group 20 serve as baselines, with Group 1 representing a fully connected graph and Group 20 representing a line coupling graph. The table shows varied results for the best performance. For the comparison between \lstinline{LookaheadSwap} and \lstinline{BasicSwap}, Group 4 performs better, with a difference of 54.22\% in swap overhead and 59.28\% in depth overhead. Conversely, for the comparison between \lstinline{LookaheadSwap} and \lstinline{SabreSwap}, Group 3 performs better, with the least negative percentage of -120.18\% for swap overhead and 28.71\% for depth overhead.
% \input{table_average_overhead}
