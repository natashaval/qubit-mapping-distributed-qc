...
class DynamicLookaheadSwap(TransformationPass):
    ...
    def list_gates_on_qubit_dag(self, dag: DAGCircuit): # TODO: LIST_GATE ALREADY layers(), but dependency list NOT YET
        ...
        for layer in dag.layers():
            for operation in layer['graph'].op_nodes():
                ...
                index += 1
        print("Init dependency list: ", dependency_list)
    ...
    def calc_effect_gate(self, node: DAGOpNode, swap_nodes: tuple, coupling_map: CouplingMap, current_layout: Layout):
        print(f"F effect: {node.op} {node.qargs} {node.cargs}")
    ...
    def sum_effect(self, gate_list: list[int], act_list: list[int], swap: tuple, coupling_map: CouplingMap, current_layout: Layout, str):
        is_remove = False
        total_sum = 0
        print(f"{str} effect gate_list_len {len(gate_list)} gate_list {gate_list} act_list_len {len(act_list)}")
        for gate_idx in gate_list:
            node = self.op_gates[gate_idx]
            print(f"{str} Effect gate[{gate_idx}] node: {node.name} {node.qargs}")
            if node.op.num_qubits == 2 and node.name not in ["barrier", "measure"]: # TODO: don't calculate if barrier or measure gate AND make sure that only lookahead for next wo-qubit gates ONLY
                ...
                if value < 0 and gate_idx in act_list:
                    print(f"{str} value {value} NEGATIVE and EQUAL on gate ID {node} {node.qargs}")
                    is_remove = True
                    return total_sum, is_remove
                elif value < 0:
                    print(f"{str} value {value} NEGATIVE on gate ID {node} {node.qargs}")
                    break
                elif value == 0 or value == 1:
                    print(f"{str} value {value} POSITIVE on gate ID {node} {node.qargs}")
                    total_sum += value
    ...
    def measure_node(self, node: DAGOpNode, current_layout: Layout, num_clbits) -> DAGOpNode:
        print(f"Node BEFORE {node.name} {node.qargs} {node.cargs}")
        # phy0 = current_layout.get_virtual_bits()[node.qargs[0]]
        # classical0 = self.property_set['layout'].get_physical_bits()[phy0]
        ...
        print(f"- node wire: {node.qargs[0]._index} logical: {log0} classical: {classical0} propset: {self.property_set['layout'].get_physical_bits()[log0]}")
        ...
        if classical0 < num_clbits:
            ...
            classical0 = self.property_set['layout'].get_physical_bits()[log0]._index
            node.cargs = (self.meas_register[classical0], )
        print(f"Node AFTER {node.name} {node.qargs} {node.cargs}")
        return node
    ...
    def run(self, dag: DAGCircuit):
        ...
        for layer in dag.layers():
            ...
            while act_list: # check if act_list is not empty
                ...
                print(f"Candi list: {candi_list} assigned_swap: {assigned_swap_list}")
                # line 27 - 29
                MCPE_cost = {}
                for swap in candi_list:
                    print(f"SWAP: {swap} with act_list: {act_list}")
                    log0, log1 = current_layout.get_physical_bits()[swap[0]]._index, current_layout.get_physical_bits()[swap[1]]._index
                    print(f"Effect to dlist_logical: {log0, log1}")
                    ...
                    print(f"-- first_sum: {first_total_sum} is_first_remove: {is_first_remove} second_sum: {second_total_sum} is_second_remove: {is_second_remove}")
                ...
                if MCPE_cost:
                    ...
                    if selected_value > 0: # add check only if worth it to do swap, if not will do recursive swap
                        print(f"SWAP selected: {selected_swap} value: {selected_value}")
                ...
                # print(f"Curr_layout: {current_layout} order: {order}")
                else:
                    print("MCPE_cost is empty, cannot add swap gate (?)")
        return new_dag